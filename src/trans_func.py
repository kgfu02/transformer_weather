# file containing functions that need to be imported
import collections
import logging
import os
import pathlib
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training,
        ...
        seq_len = tf.shape(x)[1] #might be buggy missing batch size dim
        attention_weights = {}

        x = tf.expand_dims(x,axis=-1)
        x = self.embedding(x)  # (batch_size, target_seq_len, d_model)
        x = tf.reshape(x,[tf.shape(x)[0],seq_len,-1])
        x *= tf.math.sqrt(tf.cast(self.d_model, tf.float32))
        x += self.pos_encoding[:, :seq_len, :]
...
def create_masks(inp, tar): #make sure to remove features dimension before passing in
    # Encoder padding mask
    #probably redundant, might falsely flag actual 0 vals -kevin
...
loss_object = tf.keras.losses.MeanSquaredError(reduction='none') #changed
...
def diff_function(real, pred):
    real = tf.cast(real,dtype=tf.double)
    pred=tf.cast(pred,dtype=tf.double)

    return tf.reduce_mean(tf.abs(tf.cast(real-pred[:,:,0],dtype=tf.float32)))
