...
for epoch in range(EPOCHS):
    ...
    for (batch, (inp, tar)) in enumerate(train_batches):
        ...
        if batch % 50 == 0 and (epoch != 0):
            print(f'Epoch {epoch + 1} Batch {batch} Loss {train_loss.result():.4f} '
                  f'Avg_diff {train_diff.result():.4f}')
            train_x_batches = np.concatenate([x for y, x in train_batches], axis=0)
            train_y_batches = np.concatenate([y for y, x in train_batches], axis=0)
            val_diff = val_diff(trans.diff_function(evaluate(train_x_batches,train_y_batches),train_y_batches))
    ...
    print(f'Epoch {epoch + 1} Loss {train_loss.result():.4f} Avg_diff {train_diff.result():.4f}')
